...
#   dataset %>% mutate(wordCount = str_count())
# }
...
tidy_trainset %>%
  mutate(target = as.factor(target)) %>%
  ggplot() +
  geom_bar(aes(x = wordCountType, fill = target)) +
  labs(
    x = "Word Count Type",
    y = "Count",
    title = "Distribution of Length Types"
...
count_threshold <- 20
ratio_threshold <- 0.8
disasterKeywords <- keywordRatio %>%
  filter(n >= count_threshold & target == 1 & ratio >= ratio_threshold) %>%
  .$keyword

checkDisasterKeyword <- function(dataset, disasterKeywords){
  dataset %>%
    mutate(
...
tidy_trainset <- checkDisasterKeyword(tidy_trainset, disasterKeywords)
  # tidy_trainset %>%
  # mutate(
  #   containsDisasterKeyword = str_detect(
  #     keyword,
  ...
  #   T ~ "Empty"
  # ))
tidy_valid <- checkDisasterKeyword(tidy_valid, disasterKeywords)
tidy_test <- checkDisasterKeyword(tidy_test, disasterKeywords)
tidy_trainset
```
...
checkDisasterAt <- function(dataset, disasterAt){
  dataset %>%
    mutate(
...
tidy_trainset <- checkDisaterAt(tidy_trainset, disasterAt)
  # tidy_trainset %>%
  # mutate(
  #   containsDisasterAt = str_detect(
  #     at,
  ...
  #   T ~ "Empty"
  # ))
tidy_valid <- checkDisasterAt(tidy_valid, disasterAt)
tidy_test <- checkDisaterAt(tidy_test, disasterAt)
tidy_trainset
```
...
disasterWords <- wordRatio %>%
  filter(target == 1 & ratio >= ratio_threshold & n >= count_threshold & !(word %in% excluded_word)) %>%
  .$word

checkDisasterWord <- function(dataset, disasterWords){
  dataset %>%
    mutate(
...
tidy_trainset <- checkDisasterWord(tidy_trainset, disasterWords)
  # tidy_trainset %>%
  # mutate(
  #   containsDisasterWord = str_detect(
  #     text,
  ...
  #   )
  # )
tidy_valid <- checkDisasterWord(tidy_valid, disasterWords)
tidy_test <- checkDisasterWord(tidy_test, disasterWords)
...
input <- tidy_trainset %>%
  ...
  select(target, charLength, wordCount, containsDisasterKeyword, containsDisasterAt, containsDisasterWord)

library(neuralnet)
# data(iris)
# iris
network <- neuralnet(
    target ~ charLength + wordCount,
    data = input,
    hidden = 3,
    linear.output = F
    # stepmax = 10000
...
t <- table(pred.class, input$target)
t
sum(diag(t))/sum(t)
