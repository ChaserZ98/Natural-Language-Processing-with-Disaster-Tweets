...
```{r}
top20DisasterUnigrams <- unigrams %>%
  filter(target == 1) %>%
  arrange(desc(n)) %>%
  select(unigram, n) %>%
  head(20)

top20DisasterUnigrams %>%
...
top20NonDisasterUnigrams <- unigrams %>%
  filter(target == 0) %>%
  arrange(desc(n)) %>%
  select(unigram, n) %>%
  head(20)

top20NonDisasterUnigrams %>%
  ggplot(aes(reorder(unigram, n), n)) +
  geom_bar(stat = "identity") + 
  coord_flip() +
  labs(
...
```{r}
top20DisasterBigrams <- bigrams %>%
  filter(target == 1) %>%
  arrange(desc(n)) %>%
  select(bigram, n) %>%
  head(20)

top20DisasterBigrams %>%
...
top20NonDisasterBigrams <- bigrams %>%
  filter(target == 0) %>%
  arrange(desc(n)) %>%
  select(bigram, n) %>%
  head(20)

top20NonDisasterBigrams %>%
  ggplot(aes(reorder(bigram, n), n)) +
  geom_bar(stat = "identity") + 
  coord_flip() +
  labs(
...
```{r}
top20DisasterTrigrams <- trigrams %>%
  filter(target == 1) %>%
  arrange(desc(n)) %>%
  select(trigram, n) %>%
  head(20)
top20DisasterTrigrams %>%
...
top20NonDisasterTrigrams <- trigrams %>%
  filter(target == 0) %>%
  arrange(desc(n)) %>%
  select(trigram, n) %>%
  head(20)
top20NonDisasterTrigrams %>%
  ggplot(aes(reorder(trigram, n), n)) +
  geom_bar(stat = "identity") + 
  coord_flip() +
  labs(
...
# pred.class
# t <- table(pred.class$actual, input_valid$target)
# t
# sum(diag(t))/sum(t)
