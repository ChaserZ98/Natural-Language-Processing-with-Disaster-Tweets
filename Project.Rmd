...
```{r}
tidy_train %>% select(id, target, disaster_unigram_count, non_disaster_unigram_count, disaster_bigram_count, non_disaster_bigram_count, disaster_trigram_count, non_disaster_trigram_count) %>% glimpse()
```
...
nrc_hashtag <- read_table(
  './Lexicon/NRC-Hashtag-Emotion-Lexicon-v0.2/NRC-Hashtag-Emotion-Lexicon-v0.2.txt',
...
# data_score <- tidy_train %>% 
#   unnest_tokens(word,text) %>% 
#   left_join(nrc_hashtag, by='word') %>% 
#   group_by(id) %>% 
#   summarise(score= sum(score, na.rm = T))
