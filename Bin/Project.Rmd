...
```{r download source data, warning=FALSE}
...
```{r parse location, warning=FALSE}
...
```{r topic model library}
library(topicmodels)
library(textmineR)
```

```{r model tuning code, warning=FALSE}
dtm <- CreateDtm(
  tokens$text,
  doc_names = tokens$ID,
  ngram_window = c(1, 2)
...
```{r extract disaster keyword count}
...
```{r neural network library}
library(neuralnet)
```

```{r neural network train}
set.seed(25)
...
model_file_name <- "neural network.rds"
model_path <- paste(model_directory, model_file_name, sep = "")

if(file.exists(model_path)){
  network <- readRDS(model_path)
}else{
  network <- neuralnet(
    target ~
      link_count + 
  ...
  saveRDS(network, model_path)
}
network$result.matrix[1,]
plot(network, rep = "best")
```

```{r neural network valid result}
...
```{r process twitter data}
tidy_twitterData <- twitterData %>%
  extract_links() %>%
  # replace_url_special_character() %>%
  replace_new_line() %>%
  extract_at() %>%
  extract_tags() %>%
  mutate(disaster_keyword_count = 0, non_disaster_keyword_count = 0) %>%
  # checkDisasterKeyword(disaster_keywords) %>%
  # checkNonDisasterKeyword(non_disaster_keywords) %>%
  countDisasterTag(disaster_tag) %>%
  countNonDisasterTag(non_disaster_tag) %>%
...
```{r convert twitter data}
input_twitterData <- tidy_twitterData %>%
  processModelInput()
input_twitterData
```

## Predict Using Random Forest Bagging

```{r bagging on twitter data}
p1 <- predict(bagging, input_twitterData)
p1 %>%
  as.factor() %>%
  confusionMatrix(as.factor(input_twitterData$target))
```

## Predict Using Random Forest Boosting

```{r boosting on twitter data}
predict.result <- predict(boosting, input_twitterData, n.trees = 320, type = 'response')
predict.result[predict.result > 0.5] = 1
predict.result[predict.result <= 0.5] = 0
predict.result %>%
  as.factor() %>%
...
```{r neural network on twitter data}
network.results <- compute(network, input_twitterData, rep = which.min(network$result.matrix[1, ]))
results <- data.frame(actual = input_twitterData$target, prediction = round(network.results$net.result))
t <- table(results$prediction, results$actual)
t
prop.table(t)
sum(diag(t))/sum(t)
```
